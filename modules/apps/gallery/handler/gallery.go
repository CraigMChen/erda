...
func (p *GalleryHandler) ListOpus(ctx context.Context, req *pb.ListOpusReq) (*pb.ListOpusResp, error) {
	...
	var pageNo, pageSize = AdjustPaging(req.GetPageSize(), req.GetPageNo())

	// prepare options for querying opuses
	options := PrepareListOpusesOptions(orgID, req.GetType(), req.GetName(), pageSize, pageNo)
	if req.GetKeyword() != "" {
		_, versions, err := dao.ListVersions(dao.Q(), dao.WhereOption("summary LIKE ?", "%"+req.GetKeyword()+"%"))
		if err != nil {
			l.WithError(err).Errorln("failed to ListVersions")
			return nil, apierr.ListOpus.InternalError(err)
		}
		options = append(options, PrepareListOpusesKeywordFilterOption(req.GetKeyword(), versions))
	}
	...
	// query versions by opuses ids
	_, versions, err := dao.ListVersions(dao.Q(), PrepareListVersionsInOpusesIDsOption(opuses))
	if err != nil {
		l.WithError(err).Errorln("failed to ListVersions")
		return nil, apierr.ListOpus.InternalError(err)
	}

	return ComposeListOpusResp(total, opuses, versions), nil
}
...
func (p *GalleryHandler) ListOpusVersions(ctx context.Context, req *pb.ListOpusVersionsReq) (*pb.ListOpusVersionsResp, error) {
	...
	ComposeListOpusVersionRespWithOpus(&resp, opus)
	if err = ComposeListOpusVersionRespWithVersions(&resp, versions); err != nil {
		l.WithError(err).Errorln("failed to ComposeListOpusVersionRespWithVersions")
	}
	ComposeListOpusVersionRespWithPresentations(&resp, presentations)
	ComposeListOpusVersionRespWithReadmes(&resp, lang, readmes)
	return &resp, nil
}
